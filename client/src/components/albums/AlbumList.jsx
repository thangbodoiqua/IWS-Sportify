...
const AlbumList = ({ onAlbumSelect }) => {
  ...
  const checkScrollability = useCallback(() => {
    ...
    if (container) {
      ...
      setCanScrollLeft(
        hasHorizontalScrollbar && container.scrollLeft > tolerance
      );
      setCanScrollRight(
  ...
  useEffect(() => {
    ...
    if (container && !loading) {
      ...
      return () => {
        if (container) {
          // Ensure container is still valid on cleanup
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-3 text-white h-[240px]">
        Loading albums...
      </div>
    );
  }
          ...
          {albums.map((album) => (
            ...
            >
                ...
                <div className="absolute bottom-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 translate-y-2 group-hover:translate-y-0">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      onAlbumSelect(album);
                    }} // Stop propagation so it doesn't trigger the main div click
              ...
              <div className="p-3 flex flex-col flex-grow overflow-hidden">
                <h3
                  className="text-sm font-semibold text-white truncate whitespace-nowrap"
                  title={album.title}
                >
                  {album.title}
                </h3>
                <p
                  className="text-xs text-gray-400 truncate whitespace-nowrap"
